    ...
    return (
        <div className="max-w-[1240px] mx-auto px-6 py-6 animate-fade-in">
            <div className="mb-6">
                ...
                <p className="text-gray-500 text-[13px] font-medium">Investments you've saved for review</p>
            </div>
            ...
            <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/30 gap-3 flex-wrap">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-slate-900 text-[15px]">Saved Investments ({filtered.length})</h3>
                    </div>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filtered.map(item => (
                                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-6 py-4 text-[13px] font-bold text-slate-900">{item.case_number || item.id}</td>
                                        <td className="px-6 py-4 text-[13px] font-medium text-slate-600">
                                            {item.title || item.property_address || '—'}
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-bold rounded-md border border-indigo-100 uppercase tracking-tighter">
                                                {item.status || '—'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-[13px] font-medium text-slate-600">
