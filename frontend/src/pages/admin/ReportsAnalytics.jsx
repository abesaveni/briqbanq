...
export default function ReportsAnalytics() {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <select
                        ...
                        className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                    >
                    ...
                    <button
                        ...
                        className="border border-gray-300 rounded-lg px-3 py-2 text-sm hover:bg-gray-50 flex items-center gap-2 transition-colors disabled:opacity-70"
                    >
                        <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} /> Refresh
                    </button>
                    <button
                        onClick={() => exportToPDF('Platform Overview', reportTypes.flatMap(r => r.stats))}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm"
                    >
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 text-sm font-medium text-indigo-700 flex items-center gap-2">
                        ...
                        <div key={report.id} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow">
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-4">
                                {report.stats.map(stat => (
                                    <div key={stat.label} className="bg-gray-50 rounded-lg p-2.5 text-center border border-gray-100">
                            ...
                            <button
                                ...
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors"
                            >
                            ...
                            return (
                                <div key={m.month}>
                                    <div className="flex justify-between text-sm mb-1">
                                        ...
                                        <span className="font-bold text-gray-900">{fmt(m.amount)}</span>
                                    </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200">
                ...
                <div className="divide-y divide-gray-100">
                    {activity.length === 0 ? (
                        ...
                        <div key={item.id} className="px-6 py-4 flex items-start gap-3 hover:bg-gray-50 transition-colors">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${ACTIVITY_COLORS[item.type] || 'bg-gray-100 text-gray-600'}`}>
