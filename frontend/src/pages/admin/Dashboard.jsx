...
export default function Dashboard() {
    ...
    const statCards = [
        { label: 'Total Cases', value: totalCases, sub: `${activeCases} active · ${listedCases} listed`, icon: FileText, iconBg: 'bg-indigo-50', iconColor: 'text-indigo-600', growth: d.cases_growth },
        { label: 'Platform Users', value: d.total_users ?? 0, sub: `${d.pending_kyc_reviews ?? 0} pending KYC`, icon: Users, iconBg: 'bg-violet-50', iconColor: 'text-violet-600', growth: d.users_growth },
    ...
    return (
        ...
        <div className="space-y-4">
            ...
            <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">Platform administration · compliance management</p>
            </div>
                ...
                {statCards.map((card, i) => {
                    ...
                    return (
                        <div key={i} className="bg-white rounded-lg border border-slate-100 shadow-sm p-4">
                            <div className="flex items-start justify-between mb-3">
                                ...
                                {card.growth && (
                                    <span className="flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">
                                        <TrendingUp className="w-2.5 h-2.5" />{card.growth}
                                    </span>
                                )}
                            </div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-1.5 leading-tight">{card.sub}</p>
                        </div>
                    )
                })}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
                {/* Platform Status */}
                <div className="lg:col-span-2 bg-white rounded-lg border border-slate-100 shadow-sm p-4">
                    <p className="text-[11px] font-semibold text-slate-700 mb-3">Platform Status</p>
                            ...
                            <div key={i} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-[11px] font-medium text-slate-700">{row.label}</p>
                                        <p className="text-[10px] text-slate-400 leading-tight">{row.sub}</p>
                                    </div>
                                </div>
                                <span className="text-base font-bold text-slate-800 shrink-0 ml-2">{row.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Recent Cases */}
                <div className="lg:col-span-3 bg-white rounded-lg border border-slate-100 shadow-sm p-4">
                    <div className="flex items-center justify-between mb-3">
                        <p className="text-[11px] font-semibold text-slate-700">Recent Cases</p>
                        <Link to="/admin/case-management" className="flex items-center gap-0.5 text-[10px] font-semibold text-indigo-600 hover:text-indigo-700">
                            View all <ArrowUpRight className="w-3 h-3" />
                        </Link>
                    </div>
                    {recentCases.length === 0 ? (
                        <p className="text-[11px] text-slate-400 text-center py-6">No cases yet</p>
                    ) : (
                        <div className="divide-y divide-slate-50">
                            {recentCases.map((c, i) => (
                                <div key={c.id || i} className="py-2 flex items-center gap-3">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <p className="text-[11px] font-semibold text-slate-800 truncate">{c.title || c.property_address || 'Untitled Case'}</p>
                                            <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full shrink-0 ${STATUS_BADGE[c.status] || 'bg-slate-100 text-slate-500'}`}>
                                                {c.status}
                                            </span>
                                        </div>
                                    ...
                                    <p className="text-[10px] font-semibold text-slate-600 shrink-0">{c.loan_amount ? formatCurrency(c.loan_amount) : ''}</p>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div className="bg-white rounded-lg border border-slate-100 shadow-sm p-4">
                <p className="text-[11px] font-semibold text-slate-700 mb-3">Quick Actions</p>
                    ...
                    {QUICK_ACTIONS.map((action, i) => {
                        ...
                        return (
                            <button
                                ...
                                className="flex flex-col items-start p-3.5 rounded-lg border border-slate-100 hover:border-indigo-100 hover:bg-slate-50 transition-all text-left group"
                            >
                                <div className={`w-8 h-8 rounded-lg ${action.bg} flex items-center justify-center mb-2.5`}>
                                    <Icon className={`w-4 h-4 ${action.color}`} />
                                </div>
                                <p className="text-[11px] font-semibold text-slate-700 group-hover:text-slate-900">{action.label}</p>
            ...
            <div className="flex items-center justify-between bg-slate-800 rounded-lg px-4 py-3">
                <p className="text-[11px] text-slate-400">Need help? Our support team is available during business hours.</p>
                <div className="flex gap-2 shrink-0">
                    <button onClick={() => setShowSupportModal(true)} className="text-[10px] font-semibold px-3 py-1.5 rounded-md bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors">Resources</button>
                    <button onClick={() => setShowContactModal(true)} className="text-[10px] font-semibold px-3 py-1.5 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">Contact Support</button>
                </div>
            </div>
        </div>
        ...
        {showSupportModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowSupportModal(false)}>
                <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 border border-slate-100" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                        <p className="text-[11px] font-semibold text-slate-800">Support Resources</p>
                    ...
                    <div className="px-4 py-3 space-y-1">
                        {[
                            ...
                            <div key={r.name} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                                <div>
                                    <p className="text-[11px] font-semibold text-slate-700">{r.name}</p>
                    ...
                    <div className="px-4 py-3 border-t border-slate-100">
                        <button onClick={() => setShowSupportModal(false)} className="w-full text-[11px] font-semibold py-2 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors">Close</button>
                    </div>
                </div>
            </div>
        )}
        ...
        {showContactModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowContactModal(false)}>
                <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 border border-slate-100" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                        <p className="text-[11px] font-semibold text-slate-800">Contact Support</p>
                    ...
                    <div className="px-4 py-3 space-y-1">
                        {[
                            ...
                            <div key={c.label} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                                ...
                                <p className="text-[11px] font-semibold text-slate-700">{c.value}</p>
                            </div>
                        ))}
                    </div>
                    <div className="px-4 py-3 border-t border-slate-100">
                        <button onClick={() => setShowContactModal(false)} className="w-full text-[11px] font-semibold py-2 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors">Close</button>
                    </div>
                </div>
            </div>
        )}
