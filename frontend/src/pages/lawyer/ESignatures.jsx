...
import { getGovSignData } from './api'
...
export default function ESignatures() {
  ...
  const [envelopeRiskFilter, setEnvelopeRiskFilter] = useState('All Risks')
  ...
  useEffect(() => {
    const closeEnvelopeActions = (e) => {
      if (envelopeActionsOpen != null && !e.target.closest('[data-envelope-actions]')) setEnvelopeActionsOpen(null)
    }
    document.addEventListener('mousedown', closeEnvelopeActions)
    return () => document.removeEventListener('mousedown', closeEnvelopeActions)
  }, [envelopeActionsOpen])
  ...
  const filteredEnvelopes = envelopes.filter((e) => {
    ...
    if (envelopeRiskFilter !== 'All Risks' && e.risk !== envelopeRiskFilter) return false
    return true
  })
  const filteredEvidence = evidenceEvents.filter((e) => {
    if (evidenceSearch && ![e.id, e.type, e.actor, e.originIp, e.auth].some((v) => String(v || '').toLowerCase().includes(evidenceSearch.toLowerCase()))) return false
  ...
  const handleConfigureHSM = () => setShowHSMModal(true)
  ...
  const handleSubmitUploadDocument = () => {
    if (!uploadDocumentForm.name.trim()) {
      setUploadNameError('Document name is required.')
      return
    }
    setUploadNameError('')
    const fileSizeBytes = selectedUploadFile?.size || 0
    const fileSizeDisplay = fileSizeBytes > 0
  ...
  const handleGenerateReport = () => {
    ...
    if (reportDateFrom && reportDateTo && reportDateTo < reportDateFrom) {
      setReportDateError('"To date" cannot be earlier than "From date".')
      return
    }
    ...
    <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="p-6 space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl shrink-0 ${
                        card.color === 'blue' ? 'bg-blue-600' : card.color === 'orange' ? 'bg-amber-500' : card.color === 'purple' ? 'bg-purple-500' : 'bg-red-500'
                      }`}>
                        {card.value}
                      </div>
                      <div>
              ...
              <div>
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  ...
                  <button type="button" onClick={handleViewAllEnvelopes} className="text-blue-600 text-sm font-medium hover:underline">
                    View All Envelopes
                  </button>
                </div>
                      ...
                      <div key={task.id} className="rounded-lg border border-gray-200 p-4 flex flex-wrap items-center justify-between gap-4 hover:bg-gray-50/50">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {isSigned ? (
                            ...
                            <button type="button" onClick={handleReviewSign(task)} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2">
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={handleCreateEnvelope} className="px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2">
                    <span>+</span> Create Envelope
                  </button>
                  <button
                    type="button"
                    onClick={() => setShowEnvelopeFilters((v) => !v)}
                    className={`px-4 py-2.5 border rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${showEnvelopeFilters ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                    Filters
                    {(envelopeTypeFilter !== 'All Types' || envelopeStatusFilter !== 'All Statuses' || envelopeRiskFilter !== 'All Risks') && (
              ...
              {showEnvelopeFilters && (
                <div className="flex flex-wrap gap-3 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={envelopeTypeFilter} onChange={(e) => setEnvelopeTypeFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select value={envelopeStatusFilter} onChange={(e) => setEnvelopeStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-xs font-medium text-gray-600 whitespace-nowrap">Risk</label>
                    <select value={envelopeRiskFilter} onChange={(e) => setEnvelopeRiskFilter(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                  ...
                  <button type="button" onClick={() => { setEnvelopeTypeFilter('All Types'); setEnvelopeStatusFilter('All Statuses'); setEnvelopeRiskFilter('All Risks') }} className="text-xs text-gray-500 hover:text-gray-700 underline">
                    Clear filters
                  </button>
                </div>
              )}
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              <div className="w-20 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full bg-blue-600 rounded-full" style={{ width: `${(row.progressVal || 0) * 100}%` }} />
                              </div>
          ...
          {activeTab === 'Documents' && (
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="rounded-xl border-2 border-gray-200 bg-white p-5 flex items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
                    <svg className="w-7 h-7 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-gray-500 text-blue-600">In progress</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <button type="button" onClick={handleUploadDocument} className="px-4 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2 shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1 min-w-0">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                  />
                </div>
                <select value={documentTypeFilter} onChange={(e) => setDocumentTypeFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                  <option>All Types</option>
                  <option>PDF</option>
                  <option>DOCX</option>
                </select>
                <select value={documentStatusFilter} onChange={(e) => setDocumentStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                  <option>All Statuses</option>
                ...
                <button
                  type="button"
                  onClick={() => { setDocumentTypeFilter('All Types'); setDocumentStatusFilter('All Statuses'); setDocumentSearch('') }}
                  className={`px-4 py-2.5 border rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${
                    (documentTypeFilter !== 'All Types' || documentStatusFilter !== 'All Statuses')
                ...
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                  {(documentTypeFilter !== 'All Types' || documentStatusFilter !== 'All Statuses') ? 'Clear Filters' : 'Filters'}
                </button>
              </div>
                          ...
                          <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-right">
                              <div className="flex items-center justify-end gap-1">
                                <button type="button" onClick={() => handleDocumentView(doc)} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                                <button type="button" onClick={() => handleDocumentDownload(doc)} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Download"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></button>
          ...
          {activeTab === 'Templates' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleCreateTemplate} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTemplates.map((t) => (
                  <div key={t.id} className="rounded-xl border-2 border-gray-200 p-5 hover:border-blue-600/30 transition-colors flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-100">
                      <button type="button" onClick={() => handleUseTemplate(t.id)} className="px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">Use Template</button>
          ...
          {activeTab === 'Certificates & Keys' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleIssueCertificate} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2">
                  ...
                  <div key={cert.id} className={`rounded-xl border-2 p-5 ${cert.status === 'Expiring' ? 'border-amber-300 bg-amber-50/30' : 'border-green-200 bg-white'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-200">
                      ...
                      {cert.status === 'Expiring' && (
                        <button type="button" onClick={() => handleCertificateRenew(cert.id)} className="px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700">Renew</button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'Evidence Ledger' && (
            <div className="p-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={handleExportLedger} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">
                  Export Ledger Proof
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input
                    type="text"
                    placeholder="Search by ID, type, actor..."
                ...
                <select
                  ...
                  className="border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
                  <option value="All Types">All Types</option>
                  <option value="ENVELOPE_CREATED">Envelope Created</option>
                  <option value="DOCUMENT_HASH_COMPUTED">Document Hash</option>
                  <option value="RECIPIENTS_ADDED">Recipients Added</option>
                  <option value="RISK_ASSESSMENT_COMPLETED">Risk Assessment</option>
                  <option value="HSM_SIGNATURE_APPLIED">HSM Signature</option>
                </select>
                {(evidenceSearch || evidenceTypeFilter !== 'All Types') && (
                  <button type="button" onClick={() => { setEvidenceSearch(''); setEvidenceTypeFilter('All Types') }} className="px-3 py-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg hover:bg-gray-50">
                    Clear
                  </button>
                )}
              </div>
              ...
              <div className="rounded-lg border-2 border-gray-200 bg-gray-50/50 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                  <div>
                    ...
                    <select value={reportTypeFilter} onChange={(e) => setReportTypeFilter(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
                      {reportTypes.map((r) => (
                  ...
                  <div>
                    ...
                    <GlobalDatePicker
                      ...
                      onChange={(e) => { setReportDateFrom(e.target.value); setReportDateError('') }}
                    />
                  ...
                  <div>
                    ...
                    <GlobalDatePicker
                      value={reportDateTo}
                      onChange={(e) => { setReportDateTo(e.target.value); setReportDateError('') }}
                      minDate={reportDateFrom ? new Date(reportDateFrom) : undefined}
                    />
                    <p className="text-xs text-red-600 mt-1 min-h-[16px] flex items-center gap-1">
                      {reportDateError && (
                        <>
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 py-3 border-b border-gray-100 last:border-0">
                    ...
                    {!item.locked && 'enabled' in item && (
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors ${adminToggles[item.id] ? 'bg-blue-600' : 'bg-gray-200'}`}
                      >
              ...
              <div className="rounded-lg border border-gray-200 p-5">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                  ...
                  <button type="button" onClick={handleNewPolicyRule} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 flex items-center gap-2">
                  ...
                  {adminSecurity.map((s) => (
                    <div key={s.id} className="flex flex-wrap items-center justify-between gap-4 py-3 border-b border-gray-100">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors ${adminToggles[s.id] ? 'bg-blue-600' : 'bg-gray-200'}`}
                      >
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {helpLinks.map((link) => (
                      <a key={link.id} href={link.url} className="block rounded-lg border border-gray-200 p-4 hover:border-blue-600/30 hover:bg-blue-50/50 transition-colors">
                  ...
                  <div className="mt-6 rounded-lg border-2 border-gray-200 p-4">
                    ...
                    <p className="text-sm text-gray-700">Email: <a href="mailto:support@govsign.example" className="text-blue-600 hover:underline">support@govsign.example</a></p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitCreateEnvelope} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Create</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button type="button" onClick={handleConfirmSign} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Sign</button>
            </div>
          </div>
        </div>
      )}
      ...
      {certificateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <button type="button" onClick={handleCloseCertificateModal} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
      ...
      {sovereigntyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <button type="button" onClick={() => setSovereigntyModal(false)} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmitCreateTemplate} className="mt-4 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Create</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showUploadDocumentModal && (
        <div
          ...
          onClick={() => { setShowUploadDocumentModal(false); setUploadDocumentForm({ name: '', type: 'PDF', envelopeId: '' }); setSelectedUploadFile(null); setUploadNameError('') }}
        >
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-500 mb-4">Add a document to an envelope or as a standalone file</p>
            <div className="space-y-4">
              {/* File picker */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Select file</label>
                <label className={`flex flex-col items-center justify-center gap-2 w-full border-2 border-dashed rounded-xl px-4 py-6 cursor-pointer transition-colors ${selectedUploadFile ? 'border-blue-600 bg-blue-50/40' : 'border-gray-300 hover:border-blue-600 hover:bg-blue-50/20'}`}>
                  {selectedUploadFile ? (
                    <>
                      ...
                      <span className="text-sm font-medium text-blue-600">{selectedUploadFile.name}</span>
                    ...
                    <>
                      <svg className="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0l-4 4m4-4v12"/></svg>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setUploadDocumentForm((p) => ({
                        ...p,
                        name: file.name.replace(/\.[^/.]+$/, ''),
                        type: ext === 'DOCX' || ext === 'DOC' ? 'DOCX' : 'PDF',
                      }))
                      setUploadNameError('')
                    }}
                  />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => { setUploadDocumentForm((p) => ({ ...p, name: e.target.value })); setUploadNameError('') }}
                  placeholder="e.g. Contract_2026"
                  className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 ${uploadNameError ? 'border-red-400' : 'border-gray-300'}`}
                />
                {uploadNameError && <p className="text-xs text-red-500 mt-1">{uploadNameError}</p>}
              </div>

              {/* Type */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">File type</label>
                <select value={uploadDocumentForm.type} onChange={(e) => setUploadDocumentForm((p) => ({ ...p, type: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
              ...
              {/* Link to envelope */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Link to envelope <span className="font-normal text-gray-400">(optional)</span></label>
                <select value={uploadDocumentForm.envelopeId || ''} onChange={(e) => setUploadDocumentForm((p) => ({ ...p, envelopeId: e.target.value }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-600">
            ...
            <div className="flex gap-3 mt-6 pt-4 border-t border-gray-100">
              <button
                type="button"
                onClick={() => { setShowUploadDocumentModal(false); setUploadDocumentForm({ name: '', type: 'PDF', envelopeId: '' }); setSelectedUploadFile(null); setUploadNameError('') }}
                className="flex-1 border border-gray-300 bg-white text-gray-700 text-sm font-medium py-2.5 rounded-lg hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2.5 rounded-lg flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0l-4 4m4-4v12"/></svg>
                Upload Document
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSaveEditTemplate} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Save</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="view-envelope-title">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={() => setViewingEnvelope(null)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="view-document-title">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={() => setViewingDocument(null)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="issue-certificate-title" onClick={closeIssueCertificateModal}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitIssueCertificate} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Issue Certificate</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="new-policy-rule-title" onClick={closeNewPolicyRuleModal}>
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button type="button" onClick={handleSubmitNewPolicyRule} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Add Rule</button>
            </div>
          </div>
        </div>
      )}

      {/* HSM Configuration Modal */}
      {showHSMModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="hsm-modal-title">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 id="hsm-modal-title" className="text-lg font-semibold text-gray-900">Configure HSM Cluster</h3>
              <button type="button" onClick={() => setShowHSMModal(false)} className="p-1.5 text-gray-400 hover:text-gray-600 rounded" aria-label="Close">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="space-y-4 text-sm">
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                ...
                <span className="text-green-700 text-xs">{hsmCluster.primary} — {hsmCluster.certified}</span>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Primary Cluster</label>
                <input type="text" defaultValue={hsmCluster.primary} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" readOnly />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Secondary Cluster</label>
                <input type="text" defaultValue={hsmCluster.secondary} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" readOnly />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Region</label>
                <input type="text" defaultValue={hsmCluster.region} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" readOnly />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Certification</label>
                <input type="text" defaultValue={hsmCluster.certified} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" readOnly />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Key Algorithm</label>
                <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                  <option>RSA-4096</option>
                  <option>EC P-384</option>
                  <option>EC P-521</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Auto-Failover</label>
                <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            ...
            <div className="flex gap-2 mt-6">
              <button type="button" onClick={() => setShowHSMModal(false)} className="flex-1 border border-gray-300 bg-white text-gray-700 text-sm font-medium py-2 rounded-lg hover:bg-gray-50">Cancel</button>
              <button type="button" onClick={() => { localStorage.setItem('hsmCluster', JSON.stringify(hsmCluster)); setShowHSMModal(false); }} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Save Configuration</button>
            </div>
          </div>
        </div>
      )}
      ...
      {certificateDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-auto">
            ...
            <button type="button" onClick={() => setCertificateDetail(null)} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded-lg">Close</button>
          </div>
        </div>
      )}
    </div>
