...
import { dealsService, activityService } from "../../api/dataService";
...
export default function LenderBuyNowRoom() {
    const { id } = useParams();
    ...
    const galleryImages = images.length > 0 ? images : [deal.image || PROPERTY_PLACEHOLDER];
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                addNotification({
                    type: 'success',
                    title: 'Lender Acquisition Successful',
                    message: `You have successfully purchased ${deal.title}. We will contact you shortly to finalize the asset recovery.`,
                });

                // Platform-wide Activity Log
                activityService.logActivity({
                    title: `Asset Purchased: ${deal.title}`,
                    details: `You completed a fixed-price acquisition of ${deal.title} for ${formatCurrency(deal.buyNowPrice)}`,
                    type: "completion"
                });
            ...
            addNotification({
                type: 'error',
                title: 'Error',
                message: err.message || "An unexpected error occurred during purchase"
            });
        }
    };
        ...
        <div className="max-w-[1600px] mx-auto pt-2 px-6 pb-6 space-y-5 animate-fade-in font-sans text-gray-900">
            ...
            <div className="relative h-[250px] md:h-[350px] lg:h-[400px] rounded-[2rem] overflow-hidden group shadow-xl">
                ...
                <div className="absolute top-5 left-5 md:top-8 md:left-8">
                    <div className="bg-green-600 text-white font-black text-[9px] md:text-xs uppercase tracking-widest px-3 py-1.5 md:px-4 md:py-2.5 rounded-lg flex items-center gap-2 shadow-lg border border-white/20">
                ...
                {/* Content Overlay */}
                <div className="absolute bottom-0 left-0 p-5 md:p-10 md:pl-20 w-full flex justify-between items-end">
                    ...
                    {/* Floating Price Card */}
                    <div className="bg-gradient-to-br from-green-600 to-green-600 p-4 md:p-6 rounded-2xl shadow-2xl text-white w-[180px] md:w-[260px] hidden lg:block transform translate-y-2 group-hover:translate-y-0 transition-transform border border-white/10">
                        <p className="text-white/80 font-bold text-[8px] uppercase tracking-widest mb-1 text-center font-sans">Fixed Purchase Price</p>
                        <p className="text-2xl md:text-4xl font-black text-center tracking-tighter mb-2 font-sans">{formatCurrency(deal.buyNowPrice / 1000, 0)}k</p>
                        <p className="text-[10px] md:text-sm font-bold text-white/90 text-center uppercase tracking-wider font-sans">{metrics.timeToSettlement || "45 Day Settlement"}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="grid lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                                <SectionHeader icon={<TrendingUp size={20} />} title="Loan Details" />
                            ...
                            {/* Available Documents */}
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                                <SectionHeader icon={<FileText size={20} />} title="Available Documents" />
                                <div className="grid gap-4 mt-6">
                                    {(deal?.documents || []).map((doc, idx) => (
                                        <div key={idx} className="flex justify-between items-center p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors group">
                                            <div className="flex items-center gap-4">
                                                <div className="bg-indigo-50 p-2.5 rounded-lg text-indigo-600 group-hover:scale-110 transition-transform">
                                                    <FileText size={20} />
                                                </div>
                                                <div>
                                            ...
                                            <div className="flex gap-2">
                                                <IconButton icon={<CheckCircle2 size={16} />} text="View" />
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                ...
                                <div className="p-6">
                                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
                                        <h5 className="text-blue-900 font-bold text-xs uppercase tracking-wide mb-3">Included:</h5>
                                    ...
                                    {!agreedToTerms && (
                                        <p className="text-[11px] text-amber-600 font-semibold text-center">
                                            ↑ Tick the checkbox above to enable purchase
                                        </p>
                                    )}
                                    <button
                                    ...
                                    >
                                        <Lock size={16} />
                                        {deal.status === "Sold" ? "Asset Recovered" : `Purchase Now — ${formatCurrency(deal?.buyNowPrice || 0)}`}
                                    </button>
