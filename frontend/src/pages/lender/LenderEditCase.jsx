...
export default function LenderEditCase() {
    const { id } = useParams();
    ...
    if (error) return (
        <div className="p-6">
            ...
            <button onClick={() => navigate("/lender/my-cases")} className="mt-4 flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900">
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-5 pb-10">
            <div className="flex items-center gap-3">
                <button onClick={() => navigate("/lender/my-cases")} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
                    <ArrowLeft size={18} />
                </button>
                <div>
            ...
            <form onSubmit={handleSave} className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
                ...
                <div className="p-5 space-y-4">
                    <Field label="Description">
                        <textarea name="description" value={form.description} onChange={handleChange} rows={4}
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 resize-none"
                ...
                <div className="p-5 flex items-center justify-end gap-3">
                    <button type="button" onClick={() => navigate("/lender/my-cases")}
                        className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
                        Cancel
                    </button>
                    <button type="submit" disabled={saving}
                        className="flex items-center gap-2 px-5 py-2 bg-[#1B3A6B] text-white text-sm font-semibold rounded-lg hover:bg-[#142d55] transition-colors disabled:opacity-60">
