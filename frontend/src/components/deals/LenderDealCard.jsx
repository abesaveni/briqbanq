...
export default function LenderDealCard({ deal }) {
  ...
  const dest = isBuyNow ? `/lender/buy-now/${deal.id}` : `/lender/auctions/${caseRouteId}`;
    ...
    >
      ...
      <div className="p-3 flex flex-col flex-1 gap-2">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px] border-t border-slate-100 pt-2">
          ...
          <div>
            <p className="text-slate-400 font-medium uppercase tracking-wide text-[9px]">{isLive ? "Current Bid" : "Type"}</p>
        ...
        <div className="flex gap-2 mt-auto">
          {isLive ? (
            ...
            <button
              onClick={(e) => { e.stopPropagation(); navigate(dest); }}
              className="w-full border border-slate-200 hover:bg-slate-50 text-slate-700 py-1.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5"
            >
