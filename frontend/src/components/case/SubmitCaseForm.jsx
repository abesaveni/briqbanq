...
const cardBodyCls = 'p-4 space-y-4'
...
export default function SubmitCaseForm({ role = 'lender', onClose, onSuccess }) {
  ...
  return (
    <div className={isModal ? 'flex flex-col h-full overflow-hidden' : 'max-w-4xl mx-auto pb-20 pt-6'}>
      ...
      <div className={`shrink-0 ${isModal ? 'px-5 pt-3 pb-3 border-b border-slate-100' : 'mb-5 px-1'}`}>

        {/* Row 1: title + status + save indicator + close */}
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0 flex-wrap">
            <h2 className="text-base font-bold text-slate-900">New MIP Case</h2>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 ${statusCls}`}>{statusLabel}</span>
            {lastSavedAt && !isSavingDraft && !isDirty && (
              <span className="text-xs text-slate-400 flex items-center gap-1 shrink-0"><Clock className="w-3 h-3" /> {lastSavedAt.toLocaleTimeString()}</span>
            )}
            {isSavingDraft && <span className="text-xs text-indigo-400 flex items-center gap-1 shrink-0"><RefreshCw className="w-3 h-3 animate-spin" /> Saving…</span>}
            {isDirty && !isSavingDraft && <span className="text-xs text-amber-500 flex items-center gap-1 shrink-0"><AlertTriangle className="w-3 h-3" /> Unsaved</span>}
          </div>
          {isModal && (
        ...
        {/* Row 2: progress bar inline with % */}
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className="text-xs font-semibold text-indigo-600 shrink-0 w-8 text-right">{completionPct}%</span>
        </div>

        {/* Row 3: Stepper (no per-step labels — current step name shown inline) */}
        <div className="flex items-center gap-0 overflow-x-auto mt-3">
          {STEPS.map((s, idx) => {
            ...
            return (
              <div key={s.id} className="flex items-center shrink-0">
                ...
                {idx < STEPS.length - 1 && (
                  <div className={`w-4 h-px ${currentStep > s.id ? 'bg-indigo-400' : 'bg-slate-200'}`} />
                )}
              </div>
            )
          })}
          <span className="ml-3 text-xs font-semibold text-indigo-600 shrink-0">
      ...
      {/* ── Scrollable body ────────────────────────────────────────────── */}
      <div className={isModal ? 'flex-1 overflow-y-auto px-5 py-4' : 'mb-10'}>
        {stepContent}
      </div>

      {/* ── Sticky footer navigation ───────────────────────────────────── */}
      {!submitSuccess && (
        <div className={`shrink-0 flex items-center justify-between ${isModal ? 'px-5 py-3 border-t border-slate-100 bg-white' : 'border-t border-gray-100 pt-6 px-1'}`}>
